...
(function installSecondSquareHotkey() {
  ...
  function updateConfig(nextConfig = {}) {
    ...
    persistConfig();
    if (!normalizeSlot(config.hotbarSlot)) stopTimer();
    refreshUi();
  }
  ...
  function refreshUi() {
    ...
    if (label) {
      // No configured Square #2 hotkey means Square #2 is completely disabled.
      // Do not perform any monster/X-ray scan just to update its status.
      if (!normalizeSlot(config.hotbarSlot)) {
  ...
  function tickSecondSquare() {
    if (!primaryAoeEnabled() || !normalizeSlot(config.hotbarSlot)) {
      stopTimer();
      refreshUi();
      return;
    }
