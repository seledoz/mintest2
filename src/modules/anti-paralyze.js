...
window.__minibiaBotBundle.installAntiParalyzeModule = function installAntiParalyzeModule(bot) {
  ...
  function getElementSignals(element) {
    ...
    const values = [
      element.id, element.className, element.getAttribute("title"), element.getAttribute("aria-label"),
      element.getAttribute("alt"), element.getAttribute("name"), element.getAttribute("src"),
      element.getAttribute("data-condition"), element.getAttribute("data-status"),
      element.getAttribute("data-effect"), element.getAttribute("data-tooltip"), element.getAttribute("style"),
    ];
    try { values.push(window.getComputedStyle(element).backgroundImage); } catch (_error) {}
  ...
  function findParalyzeStatusIcon() {
    ...
    const directSelector = [
      '[title*="paraly" i]', '[aria-label*="paraly" i]', '[alt*="paraly" i]', '[id*="paraly" i]',
      '[class*="paraly" i]', '[src*="paraly" i]', '[data-condition*="paraly" i]',
      '[data-status*="paraly" i]', '[data-effect*="paraly" i]', '[title*="paralis" i]',
      '[aria-label*="paralis" i]', '[alt*="paralis" i]', '[src*="paralis" i]'
    ].join(",");
    ...
    for (const element of candidates) {
      if (!isVisible(element) || !PARALYZE_PATTERN.test(getElementSignals(element))) continue;
      state.detectedElement = element;
  ...
  function tryAntiParalyze(now = Date.now()) {
    ...
    if (shouldPrioritizeHpHeal()) { bot.heal?.tryHeal?.(); return false; }
  ...
  function tick() {
    if (!state.running || !config.enabled) return;
    try { tryAntiParalyze(); }
    catch (error) { bot.log("anti-paralyze tick failed", error?.message || error); }
  ...
  function status() {
    const active = !!state.running && !!config.enabled;
    const paralyzed = active ? isParalyzedActive() : false;
    return {
      running: state.running,
      config: { ...config },
      paralyzed,
      healPriorityActive: active ? shouldPrioritizeHpHeal() : false,
      detectionSource: active ? state.detectionSource : null,
      detectedElement: active && state.detectedElement ? {
