...
window.__minibiaBotBundle.installAntiParalyzeModule = function installAntiParalyzeModule(bot) {
  ...
  const config = Object.assign({ enabled: false, spellWords: "", ignoreMonsterGuard: false, tickMs: 500, recastCooldownMs: 2100 }, bot.storage.get(configStorageKey, {}));
  config.ignoreMonsterGuard = !!config.ignoreMonsterGuard; config.tickMs = 500; config.recastCooldownMs = 2100;
  ...
  function scheduleNextTick() { if (!state.running || !config.enabled) return; state.timerId = window.setTimeout(tick, 500); }
