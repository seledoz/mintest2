...
window.__minibiaBotBundle.installAutoAttackStickyTargetModule = function installAutoAttackStickyTargetModule(bot) {
  ...
  const state = {
    timerId: null,
    lastTargetId: null,
    lastPriorityTargetId: null,
    uiTimerId: null,
  };

  function persist() {
    bot.storage.set(storageKey, { enabled: config.enabled });
  }
  ...
  function getVisibleMonsterById(id) {
    ...
    return (bot.xray?.getVisibleMonsters?.({ sameFloorOnly: true }) || [])
      .find((monster) => Number(monster?.id) === Number(id)) || null;
  }

  function getPriorityIndex(monster) {
    return Number.isFunction?.(bot.attackPriority?.getPriorityIndex)
      ? bot.attackPriority.getPriorityIndex(monster)
      : (typeof bot.attackPriority?.getPriorityIndex === "function"
        ? bot.attackPriority.getPriorityIndex(monster)
        : -1);
  }

  function selectTarget(target) {
    if (!target || !window.gameClient?.player || typeof window.gameClient.send !== "function" || typeof TargetPacket !== "function") {
      return false;
    }
  ...
  function syncPriorityOverride() {
    ...
    // Creature Priority is allowed to override Sticky Target only when the new
    // target has a strictly higher priority rank. Highest HP alone never does.
    ...
    // Same/lower priority, or an unlisted Highest-HP target: restore the sticky target.
  ...
  function syncTimer() {
    if (!config.enabled || !bot.attack?.status?.().running) {
      stopTimer();
      return;
    }
  ...
  function updateConfig(nextConfig = {}) {
    if (Object.prototype.hasOwnProperty.call(nextConfig, "enabled")) {
      ...
      if (!config.enabled) {
        state.lastTargetId = null;
        state.lastPriorityTargetId = null;
      } else {
  ...
  if (originalStart) {
    bot.attack.start = (...args) => {
      ...
      if (config.enabled) rememberCurrentTarget();
      syncTimer();
      return result;
    };
  }
  ...
  if (originalStop) {
    bot.attack.stop = (...args) => {
      ...
      stopTimer();
      if (!config.enabled) state.lastTargetId = null;
      return result;
    };
  }

  if (originalTryAttack) {
    bot.attack.tryAttack = (...args) => {
      if (config.enabled) rememberCurrentTarget();
      const result = originalTryAttack(...args);
      if (config.enabled) {
        if (state.lastTargetId == null) rememberCurrentTarget();
        restoreStickyTarget();
      }
      return result;
    };
  ...
  bot.attackStickyTarget = {
    ...
    destroy: () => {
      ...
      document.getElementById("minibia-bot-auto-attack-sticky-target-row")?.remove();
    },
  };
