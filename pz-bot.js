(() => {
  ...
  function ensureLeftCollapseButton() {
    ...
    if (!leftButton) {
      ...
      leftButton.addEventListener("click", (event) => {
        event.preventDefault(); event.stopPropagation(); rightButton.click(); window.setTimeout(syncCollapseButtons, 0);
      });
    }
    if (!document.__minNewCollapseButtonSyncInstalled) {
      document.__minNewCollapseButtonSyncInstalled = true;
      document.addEventListener("click", (event) => { if (event.target?.closest?.("#minibia-bot-collapse")) window.setTimeout(syncCollapseButtons, 0); });
    }
    syncCollapseButtons();
  }
  ...
  function removePanelDebugSection() {
    ...
    if (debugSection) { debugSection.remove(); return; }
  ...
  function removePanicRunnerSection() {
    ...
    if (panicSection) { panicSection.remove(); return; }
  ...
  function keepPanelTitleBlank() {
    blankPanelTitle(); ensureLeftCollapseButton(); removePanelDebugSection(); removePanicRunnerSection();
    let attempts = 0;
    const timerId = window.setInterval(() => {
      blankPanelTitle(); ensureLeftCollapseButton(); removePanelDebugSection(); removePanicRunnerSection(); purgeLegacyCaveWaitDelay();
      attempts += 1; if (attempts >= 20) window.clearInterval(timerId);
    }, 250);
  }
  ...
  async function loadSourceFile(path) {
    ...
    try { (0, eval)(`${code}\n//# sourceURL=${sourceUrl}`); }
    catch (error) { console.error(`[minibia-bot] Failed to evaluate ${path}`, error); throw error; }
  }

  async function load() {
    purgeLegacyCaveWaitDelay();
    if (window.minibiaBot?.destroy) { try { window.minibiaBot.destroy(); } catch (error) { console.warn("[minibia-bot] Existing bot cleanup failed", error); } }
    purgeLegacyCaveWaitDelay(); installUiCompatibilityShim(); delete window.__minibiaBotBundle; window.__minibiaBotBundle = {};
    for (const path of sourceFiles) await loadSourceFile(path);
    purgeLegacyCaveWaitDelay(); keepPanelTitleBlank(); console.log(`[minibia-bot] Loaded source files from ${repository}@${ref}`);
  }
