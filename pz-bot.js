(() => {
  ...
  function ensureFreshModuleCollapseUi(){\n    const panel=document.getElementById("minibia-bot-panel"); if(!panel)return;\n    Array.from(panel.querySelectorAll(".mb-section")).forEach((section)=>{\n      if(section.dataset.mbFreshCollapse==="true")return;\n      const label=Array.from(section.children).find((el)=>el.classList?.contains("mb-label")); if(!label)return;\n      const header=document.createElement("div"); header.className="mb-module-header"; header.style.cssText="display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:7px";\n      const button=document.createElement("button"); button.type="button"; button.className="mb-module-collapse"; button.textContent="−"; button.title="Collapse"; button.setAttribute("aria-label","Collapse module");\n      header.appendChild(label); header.appendChild(button);\n      const body=document.createElement("div"); body.className="mb-module-body";\n      Array.from(section.children).forEach((child)=>{if(child!==label)body.appendChild(child);});\n      section.replaceChildren(header,body); section.dataset.mbFreshCollapse="true";\n      button.addEventListener("click",(event)=>{event.preventDefault();event.stopPropagation();const closed=body.hidden;body.hidden=!closed;button.textContent=closed?"−":"+";button.title=closed?"Collapse":"Expand";button.setAttribute("aria-label",closed?"Collapse module":"Expand module");});\n    });\n  }\n\n  function keepPanelTitleBlank(){blankPanelTitle();ensureLeftCollapseButton();removePanelDebugSection();removePanicRunnerSection();let attempts=0;const timerId=window.setInterval(()=>{blankPanelTitle();ensureLeftCollapseButton();removePanelDebugSection();removePanicRunnerSection();purgeLegacyCaveWaitDelay();attempts+=1;if(attempts>=20)window.clearInterval(timerId);},250);}
