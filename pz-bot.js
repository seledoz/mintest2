...
/* ============================================================
   ...
   fix/rope2-cave-exception, b3e1437 e 2026-09-16T22:26:55Z pelos valores reais
...
window.__minibiaBotBundle.versionInfo = {
  number: "2.0.0",
  branch: "fix/rope2-cave-exception",
  commit: "b3e1437",
  date: "2026-09-16T22:26:55Z"
};
...
window.__minibiaBotBundle.installCaveWaypointActionsModule = function installCaveWaypointActionsModule(bot) {
  ...
  const waitState = { active: false, presetName: null, index: -1, startedAt: 0, resumeAt: 0, timerId: null, completedKey: null };

  function normalizePresetName(value) { const normalized = String(value || "").trim().replace(/\s+/g, " "); return normalized || "Default"; }
  function normalizeAction(action) {
    if (action === ropeAction || action === rope2Action || action === rope3Action || action === shovelAction || action === waitAction) return action;
    return noopAction;
  }
  function getActivePresetName() { return normalizePresetName(bot.cave?.getActivePresetName?.()); }
  function readAllActions() { const raw = bot.storage.get(actionStorageKey, {}); return raw && typeof raw === "object" && !Array.isArray(raw) ? raw : {}; }
  function writeAllActions(next) { bot.storage.set(actionStorageKey, next); return next; }
  function getPresetActions(name = getActivePresetName()) { const allActions = readAllActions(); const actions = allActions[normalizePresetName(name)]; return Array.isArray(actions) ? actions.slice() : []; }
  ...
  function getWaypointActions() { const routeLength = bot.cave?.getRoute?.().length || 0; const actions = getPresetActions(); return Array.from({ length: routeLength }, (_, index) => normalizeAction(actions[index])); }
  function setWaypointAction(index, action) {
    ...
    const actions = getWaypointActions(); actions[normalizedIndex] = normalizeAction(action); savePresetActions(actions);
  ...
  function setLastWaypointAction(action) { const routeLength = bot.cave?.getRoute?.().length || 0; return routeLength ? setWaypointAction(routeLength - 1, action) : null; }
  function normalizePosition(value) { if (!value) return null; const x = Number(value.x), y = Number(value.y), z = Number(value.z); if (![x, y, z].every(Number.isFinite)) return null; return { x: Math.trunc(x), y: Math.trunc(y), z: Math.trunc(z) }; }
  function getPositionKey(position) { return position ? `${position.x},${position.y},${position.z}` : null; }
  function getTilePosition(tile) { return normalizePosition(tile?.__position); }
  function getThingDefinition(itemId) { if (!itemId) return null; return window.gameClient?.itemDefinitionsByCid?.[itemId] || window.gameClient?.itemDefinitionsBySid?.[itemId] || window.gameClient?.itemDefinitions?.[itemId] || null; }
  function getThingName(thing) { const definition = getThingDefinition(thing?.id); return String(definition?.properties?.name || thing?.name || "").trim().toLowerCase(); }
  function getTileThings(tile) { if (!tile) return []; const things = []; if (tile.id) things.push(tile); if (Array.isArray(tile.items)) tile.items.forEach((item) => { if (item) things.push(item); }); return things; }
  function tileHasNamedThing(tile, needle) { const value = String(needle || "").trim().toLowerCase(); return !!value && getTileThings(tile).some((thing) => getThingName(thing).includes(value)); }
  function isRopeTargetTile(tile) { return tileHasNamedThing(tile, "hole") || tileHasNamedThing(tile, "rope spot"); }
  function isShovelTargetThing(thing) { const name = getThingName(thing); return !!name && shovelTargetNamePatterns.some((pattern) => pattern.test(name)); }
  function isShovelTargetTile(tile) { return getTileThings(tile).some((thing) => isShovelTargetThing(thing)); }
  function getLoadedTiles() { const chunks = window.gameClient?.world?.chunks || []; const tiles = []; for (const chunk of chunks) { if (!chunk?.tiles) continue; for (const tile of chunk.tiles) if (tile?.__position) tiles.push(tile); } return tiles; }
  function isRopeItem(item) { const name = getThingName(item); return !!name && ropeNamePattern.test(name); }
  function isShovelItem(item) { const name = getThingName(item); return !!name && shovelNamePattern.test(name); }
  function getEquipment() { return window.gameClient?.player?.equipment || null; }
  function getOpenContainers() { return Array.from(window.gameClient?.player?.__openedContainers || []); }
  function findToolSource(predicate) {
    const equipment = getEquipment();
    if (equipment?.slots) for (let slotIndex = 0; slotIndex < equipment.slots.length; slotIndex += 1) { const item = equipment.getSlotItem?.(slotIndex); if (predicate(item)) return { which: equipment, index: slotIndex, item, location: "equipment" }; }
    for (const container of getOpenContainers()) { const slots = container?.slots || []; for (let slotIndex = 0; slotIndex < slots.length; slotIndex += 1) { const item = container.getSlotItem?.(slotIndex); if (predicate(item)) return { which: container, index: slotIndex, item, location: "container" }; } }
    return null;
  }
  function findRopeSource() { return findToolSource(isRopeItem); }
  function findShovelSource() { return findToolSource(isShovelItem); }
  function distanceOnSameFloor(a, b) { if (!a || !b || a.z !== b.z) return Number.POSITIVE_INFINITY; return Math.abs(a.x - b.x) + Math.abs(a.y - b.y); }
  function isAtWaypoint(position, waypoint) { if (!position || !waypoint || position.z !== waypoint.z) return false; const tolerance = Math.max(1, Math.trunc(Number(bot.cave?.status?.()?.config?.waypointTolerance) || 1)); return Math.abs(position.x - waypoint.x) <= tolerance && Math.abs(position.y - waypoint.y) <= tolerance; }
  function isBesideOrSameTile(a, b) { return !!a && !!b && a.z === b.z && Math.abs(a.x - b.x) <= 1 && Math.abs(a.y - b.y) <= 1; }
  function findNearestTargetTile(origin, preferredPosition = null, radius = 2, predicate = () => false) {
    if (!origin) return null; let best = null; let bestScore = Number.POSITIVE_INFINITY;
    getLoadedTiles().forEach((tile) => { const position = getTilePosition(tile); if (!position || position.z !== origin.z || !predicate(tile)) return; if (Math.abs(position.x - origin.x) > radius || Math.abs(position.y - origin.y) > radius) return; const score = distanceOnSameFloor(origin, position) * 10 + (preferredPosition ? distanceOnSameFloor(preferredPosition, position) : 0); if (score < bestScore) { bestScore = score; best = { tile, position }; } });
    return best;
  }
  function useToolOnNearestTarget({ action, tool, target, preferredPosition = null, missingToolLog, usedLog }) {
    const now = Date.now(); if (now - lastToolUseAt < 500) return true; const playerPosition = normalizePosition(bot.getPlayerPosition?.()); if (!playerPosition) return false;
    const targetEntry = findNearestTargetTile(playerPosition, preferredPosition, 2, target); if (!targetEntry || !isBesideOrSameTile(playerPosition, targetEntry.position)) return false; const toolEntry = tool(); if (!toolEntry) { bot.log(missingToolLog); return false; }
    window.gameClient?.mouse?.__handleItemUseWith?.({ which: toolEntry.which, index: toolEntry.index }, { which: targetEntry.tile, index: 0xFF }); lastToolUseAt = now;
    bot.log(usedLog, { action, source: targetEntry.position, toolLocation: toolEntry.location, toolSlot: toolEntry.index, toolName: getThingName(toolEntry.item) }); return true;
  }
  function useRopeOnNearestHole(preferredPosition = null) { return useToolOnNearestTarget({ action: ropeAction, tool: findRopeSource, target: isRopeTargetTile, preferredPosition, missingToolLog: "cave rope waypoint skipped: no rope found", usedLog: "cave waypoint used rope" }); }
  function useShovelOnNearestHole(preferredPosition = null) { return useToolOnNearestTarget({ action: shovelAction, tool: findShovelSource, target: isShovelTargetTile, preferredPosition, missingToolLog: "cave shovel waypoint skipped: no shovel found", usedLog: "cave waypoint used shovel" }); }
  function stopCurrentMovement() {
    const pathfinder = window.gameClient?.world?.pathfinder; try { pathfinder?.setPathfindCache?.(null); } catch (_) {}
    const targets = [pathfinder, window.gameClient?.player, window.gameClient?.world].filter(Boolean);
    ["stop", "cancel", "clear", "clearPath", "stopWalking", "cancelWalking", "stopAutoWalk", "reset"].forEach((name) => targets.forEach((target) => { if (typeof target?.[name] !== "function") return; try { target[name](); } catch (_) {} }));
  }
  function clearWaitTimer() { if (waitState.timerId != null) { window.clearTimeout(waitState.timerId); waitState.timerId = null; } }
  function finishWaypointWait() {
    if (!waitState.active) return false; const presetName = waitState.presetName, index = waitState.index, waitKey = `${presetName}:${index}`; clearWaitTimer(); waitState.active = false; waitState.startedAt = 0; waitState.resumeAt = 0; waitState.completedKey = waitKey;
    const actions = getWaypointActions(), samePreset = getActivePresetName() === presetName, sameWaitWaypoint = actions[index] === waitAction, manuallyPaused = !!bot.pauseBreak?.status?.()?.paused, stillEnabled = bot.cave?.config?.enabled !== false;
    if (!samePreset || !sameWaitWaypoint || manuallyPaused || !stillEnabled) { bot.log("waypoint wait finished without auto-resume", { index: index + 1, samePreset, sameWaitWaypoint, manuallyPaused, stillEnabled }); return false; }
    bot.log("waypoint wait finished", { index: index + 1, waitMs: waitDurationMs }); return !!bot.cave?.start?.();
  }
  function startWaypointWait(status, index, waypoint) {
    if (waitState.active) return true; const presetName = getActivePresetName(), waitKey = `${presetName}:${index}`; if (waitState.completedKey === waitKey) return false;
    waitState.active = true; waitState.presetName = presetName; waitState.index = index; waitState.startedAt = Date.now(); waitState.resumeAt = waitState.startedAt + waitDurationMs;
    stopCurrentMovement(); bot.cave?.stop?.({ persistEnabled: false }); stopCurrentMovement(); bot.log("waypoint wait started", { index: index + 1, waitMs: waitDurationMs, waypoint }); waitState.timerId = window.setTimeout(finishWaypointWait, waitDurationMs); return true;
  }
  function getNextRouteIndex(status) {
    const route = bot.cave?.getRoute?.() || []; if (route.length <= 1) return 0; const currentIndex = Math.max(0, Math.min(route.length - 1, Math.trunc(Number(status?.currentIndex) || 0))); let direction = Number(status?.direction) || 1; let nextIndex = currentIndex + direction; if (nextIndex >= route.length) nextIndex = route.length - 2; else if (nextIndex < 0) nextIndex = 1; return Math.max(0, Math.min(route.length - 1, nextIndex));
  }
  function runWaypointActionCheck() {
    const status = bot.cave?.status?.();
    if (waitState.completedKey && status?.running) { const currentKey = `${getActivePresetName()}:${Math.trunc(Number(status.currentIndex) || 0)}`; if (currentKey !== waitState.completedKey) waitState.completedKey = null; }
    if (!status?.running) return;
    const route = bot.cave?.getRoute?.() || [], index = Math.trunc(Number(status.currentIndex) || 0), waypoint = route[index], actions = getWaypointActions(), action = actions[index];
    if (!waypoint || action === noopAction) return;
    const playerPosition = normalizePosition(bot.getPlayerPosition?.());
    if (action === waitAction) { if (isAtWaypoint(playerPosition, waypoint)) startWaypointWait(status, index, waypoint); return; }
    // Rope waypoints are owned by their dedicated handlers. Do not run the legacy nearest-hole retry loop alongside them.
    if (action === ropeAction || action === rope2Action || action === rope3Action) return;
    const distance = distanceOnSameFloor(playerPosition, waypoint); if (!Number.isFinite(distance) || distance > 2) return;
    const actionKey = `${action}:${index}`; if (actionKey === lastHandledKey && Date.now() - lastToolUseAt < 2000) return;
    const used = action === shovelAction ? useShovelOnNearestHole(waypoint) : false;
    if (used) { lastHandledKey = actionKey; window.setTimeout(() => { const nextStatus = bot.cave?.status?.(); if (!nextStatus?.running) return; bot.cave?.setCurrentIndex?.(getNextRouteIndex(status)); }, 700); }
  }

  bot.cave.findRopeSource = findRopeSource;
  const originalAddWaypoint = bot.cave?.addWaypoint?.bind(bot.cave), originalAddWaypointCurrentSpot = bot.cave?.addWaypointCurrentSpot?.bind(bot.cave), originalRemoveLastWaypoint = bot.cave?.removeLastWaypoint?.bind(bot.cave), originalClearWaypoints = bot.cave?.clearWaypoints?.bind(bot.cave), originalCreatePreset = bot.cave?.createPreset?.bind(bot.cave), originalLoadPreset = bot.cave?.loadPreset?.bind(bot.cave), originalSavePreset = bot.cave?.savePreset?.bind(bot.cave);
  if (originalAddWaypoint) bot.cave.addWaypoint = (waypoint, options = {}) => { const added = originalAddWaypoint(waypoint); if (added) setLastWaypointAction(options.action); return added; };
  if (originalAddWaypointCurrentSpot) bot.cave.addWaypointCurrentSpot = (options = {}) => { const added = originalAddWaypointCurrentSpot(); if (added) setLastWaypointAction(options.action); return added; };
  if (originalRemoveLastWaypoint) bot.cave.removeLastWaypoint = () => { const removed = originalRemoveLastWaypoint(); if (removed) savePresetActions(getWaypointActions().slice(0, -1)); return removed; };
  if (originalClearWaypoints) bot.cave.clearWaypoints = () => { const result = originalClearWaypoints(); savePresetActions([]); return result; };
  if (originalCreatePreset) bot.cave.createPreset = (...args) => { const result = originalCreatePreset(...args); if (result) savePresetActions(getWaypointActions(), args[0]); return result; };
  if (originalLoadPreset) bot.cave.loadPreset = (...args) => { const result = originalLoadPreset(...args); if (result) savePresetActions(getWaypointActions(), args[0]); return result; };
  if (originalSavePreset) bot.cave.savePreset = (...args) => { const result = originalSavePreset(...args); if (result) savePresetActions(getWaypointActions(), args[0]); return result; };
  bot.cave.getWaypointActions = getWaypointActions; bot.cave.setWaypointAction = setWaypointAction; bot.cave.setLastWaypointAction = setLastWaypointAction;
  const timerId = window.setInterval(runWaypointActionCheck, 100);
  bot.addCleanup?.(() => { window.clearInterval(timerId); clearWaitTimer(); if (bot.cave.findRopeSource === findRopeSource) delete bot.cave.findRopeSource; });
};

if (window.minibiaBot) { try { window.__minibiaBotBundle.installCaveWaypointActionsModule(window.minibiaBot); } catch (error) { console.error("[minibia-bot] cave waypoint actions failed", error); } }
window.__minibiaBotBundle = window.__minibiaBotBundle || {};
...
})(window.__minibiaBotBundle);window.__minibiaBotBundle = window.__minibiaBotBundle || {};
